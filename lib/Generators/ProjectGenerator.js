...
class ProjectGenerator {
  ...
  /**
   * Create temp directories and download project assets
   */
  createTempFiles(project, socket) {
    const dir = `${appRoot}/temp/${project._id}`;
    return new Promise((resolve, reject) => {
      async.waterfall([
        ...
        (dirs, callback) => {
          ...
          helpers.downloadSlides(socket, dirs, project.slides, callback);
        },
        (dirs, callback) => {
          console.log('Writing Project Config');
          helpers.writeFile(dirs.json + 'config.json', project, callback);
        }
      ], (e, result) => {
        if (e) {
          reject(e);
  ...
  generate(job) {
    ...
    Project.findById(id, (err, project) => {
      ...
      async.waterfall([
        ...
        Project.findByIdAndRemove(project._id, () => {});

...
class JobQueue {
    ...
    addJob(model, jobData) {
        const socket = new ioProxy(io(), jobData.socketId);
            ...
            Job.count({}, (countError, count) => {
                if (!countError) {
                    helpers.emit(socket, ['progress', {
                        message: 'Render Queue Position: ' + count
                    }]);
                }
            });
            if (this.activeJob === null) {
                this.runJob(job, socket);
            }
        });
    }
    ...
    runJob(job, socket) {
        ...
        if (job.projectId) {
            this.projectGenerator.generate(job, socket);
        } else {
            this.previewGenerator.generate(job, socket);
        }
    }
    ...
    jobComplete(job) {
        Job.findByIdAndRemove(job._id, () => {
            Job.findOne({}, null, { 'sort': { 'timestamp': 1 } }, (e, job) => {
                if (job) {
                    this.runJob(job, new ioProxy(io(), job.socketId));
                } else {
                    this.activeJob = null;

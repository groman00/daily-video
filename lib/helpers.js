...
module.exports.downloadSlides = function(socket, dir, slides, callback) {
  ...
  try {
    ...
    slides.forEach((slide, i) => {
      ...
      localPath = [dir, slide.id, '_', this.cleanImageFileName(slide.image)].join('');
      ((s, path) => {
        this.download(s.image_url_large, path, () => {
...
module.exports.cleanImageFileName = function (name) {
  return decodeURIComponent(name).split('/').pop().toLowerCase().match(/.*\.(?:gif|jpg|jpeg|png)/)[0];
}
